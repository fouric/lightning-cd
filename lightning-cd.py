...
def getFileColors(mode, selected, thisFile, fileList):# {{{
    if mode == SEARCH and thisFile in selected and showDeselectedFiles:
...
def showThisFile(thisFile, mode, selected):# {{{
    return showDeselectedFiles or mode == NORMAL or thisFile in selected or selected == []# }}}
def writePath(filename, path):# {{{
    f = open(filename, 'w+')
    f.write(path)
    f.close()# }}}
def selectedValueForMode(mode):# {{{
    return [] if mode == SEARCH else 0# }}}
...
def takeActionOnPath(f, path):# {{{
    ...
    if os.path.isdir(f):
        ...
        selected = 0 if newMode == NORMAL else []
        searchBuffer = ''
        return (newMode, selected, searchBuffer)
...
try:# {{{
    ...
    while True:
        ...
        modeText = 'search' if mode == SEARCH else 'normal'
        ...
        if keycode == termbox.KEY_SPACE:
            ...
            mode, selected, searchBuffer = takeActionOnPath(files[0 if mode == SEARCH else selected], os.path.realpath('.'))
            files = None
        elif letter == '"':
            files = None
        elif mode == NORMAL:
